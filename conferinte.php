...
$sql = "INSERT INTO REZERVARE (tipRezervare, dataSosire, dataPlecare, ora, ora2, nume, prenume, adresa, telefon, email) values('$tip', '$arrDate', '$arrDate', '$startTime', '$endTime', '$nume', '$prenume', '$adresa', '$tel', '$email')";
$sql2 = "SELECT COUNT(idRezervare) FROM `REZERVARE` WHERE tipRezervare = '$tip' AND (((ora >= '$startTime' AND ora < '$endTime') OR (ora2 > '$startTime' AND ora2 <= '$endTime')) AND dataSosire = '$arrDate')";
...
if($endTime > $startTime)
{
  ...
  {
  echo '<center>'."Intervalul orar selectat nu este disponibil!".'</center>';
  }
  else{
...
{
  echo '<center>'."Ora încheierii este invalidă!".'</center>';
}
